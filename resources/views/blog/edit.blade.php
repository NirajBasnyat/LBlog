...
@section('content')
    ...
    <x-breadcrumb listRoute="{{route('blogs.index')}}" model="Blog" :item="'Edit'"></x-breadcrumb>

    <div class="card">
        <div class="card-body">
            <h4 class="card-title">Edit Blog</h4><br>

            <x-form.wrapper action="{{route('blogs.update', $blog->id)}}" method="POST" enctype="multipart/form-data">

                @method('PATCH')

                <x-form.input type="text" label="Title" id="title" name="title" value="{{$blog->title}}"/>

                <x-form.textarea label="Content" id="content" name="content" value="{!!  $blog->content!!}" rows="5" cols="5"/>
                    ...
                    <div class="col-md-10">
                        ...
                        <select class="form-control select2" name="categories[]" id="categories" multiple>
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ in_array($category->id, old('categories', [])) || $blog->categories->contains($category->id)? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
